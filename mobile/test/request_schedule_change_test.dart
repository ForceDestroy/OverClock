...
void main() {
  testWidgets('Request Schedule Change', (WidgetTester tester) async {
    await tester.runAsync(() async {
      // Build our app and trigger a frame.
      final client = MockClient();
      ScheduleChange.httpClient = client;
      ...
      //Load screen
      await tester.pumpWidget(const MaterialApp(home: ScheduleChange()));
      ...
      //look for reason text field
      expect(find.byKey(const ValueKey("reason")), findsOneWidget);

      //look for submit request button
      expect(find.byKey(const ValueKey("scheduleChange")), findsOneWidget);
      ...
      //look for reason text field padding
      expect(find.byKey(const ValueKey("txtFieldPadding")), findsOneWidget);

      //look for reason text field sized box
      expect(find.byKey(const ValueKey("txtFieldBox")), findsOneWidget);

      await tester.pump(const Duration(milliseconds: 100));
      final requestTextField = find.byKey(const ValueKey("reasonTxtField"));
      await tester.ensureVisible(requestTextField);
      await tester.tap(find.byKey(const ValueKey("reasonTxtField")));
      await tester.pumpAndSettle();

      //look for submit request padding
      expect(find.byKey(const ValueKey("scheduleChangePad")), findsOneWidget);
      await tester.pump(const Duration(milliseconds: 100));
      final requestButton = find.byKey(const ValueKey("scheduleChange"));
      await tester.ensureVisible(requestButton);
      await tester.tap(find.byKey(const ValueKey("scheduleChange")));
      await tester.pumpAndSettle();
    });
  });
